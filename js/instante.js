...
var Instante = (function () {
  ...
  function dibujar(cx, i, t) {
    ...
    if (i.activo && !i.resuelto) {
      ...
      cx.strokeStyle = 'rgba(236,206,140,' + (.20 + .22 * pulso).toFixed(3) + ')';
      cx.lineWidth = 2;
      cx.beginPath();
      ...
      if (f > 0) {
        cx.strokeStyle = 'rgba(255,240,200,' + (.55 + .45 * f).toFixed(3) + ')';
        cx.lineWidth = 3 + 4 * f;
        cx.lineCap = 'round';
        cx.beginPath();
      ...
      if (f > .5) {
        ...
        g.addColorStop(.7, 'rgba(255,230,170,' + (.20 * (f - .5) * 2).toFixed(3) + ')');
