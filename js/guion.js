...
var Guion = (function () {
  ...
  var LUGARES = {
    ...
    calesita: {
      ...
      vuelta: 'La calesita otra vez, con los mismos caballitos en el mismo ' +
              ...
              'mitad de subida.',
      /* El simbolo ya estaba en el dibujo y el texto no lo decia: gira sin
         apoyarse y no deja marca. Eso es seguir andando por inercia — hacer
         todo, dar la vuelta entera, y no estar apoyada en nada. */
      esconde: 'No está apoyada. Se levantó un dedo del piso y sigue girando ' +
               'ahí arriba, con las luces dando toda la vuelta para el mismo ' +
               'lado, como si nada. Yo hace rato que ando igual: sigo dando ' +
               'vueltas sin apoyarme en nada y nadie se dio cuenta. Cuando ' +
               'aflojo la vista apoya de nuevo, y en la tierra no queda marca.',
      revela: 'arbol'
    },
    laguna: {
      nombre: 'El agua',
      ...
      /* Termina en la espera y no en el resultado. "No aparece" contestaba la
         pregunta que la frase anterior deja abierta, y de paso repetia lo que
         el parrafo ya dijo en la primera linea. Cortado en "por si aparece
         tarde" el texto se acaba donde ella todavia esta mirando. */
    ...
    circulo: {
      ...
      /* No baja solo, y ese era el error: lo que pasa de verdad en un momento
         ...
         que mirarlo. */
      /* El texto nombra lo que se esta ESCUCHANDO.

         Decia "el corazon me golpea en las orejas", que es una imagen y nada
         mas. Pero en este lugar el latido suena de verdad —latirCirculo, de 62
         a 138 pulsaciones por minuto, acelerando con lo que aprieta— asi que
         el texto puede senalarlo en vez de inventar una metafora al lado.

         Y "me estoy muriendo aca adentro y no hay nadie" paso a ser "siento
         que no salgo mas de aca", que lo escribio Nico y es mejor: encadena
         con el "esto no baja, no baja nunca" que viene justo antes, dice lo
         mismo sin tener que nombrarlo, y es lo que de verdad se piensa. */
      esconde: 'Los colores se ponen mejores y mejores hasta que se ponen ' +
               ...
               'todo, lo único que escucho es el corazón cada vez más rápido, ' +
               'y esto no baja. No baja nunca. Siento que no salgo más de acá. ' +
               'Y entonces veo el árbol en el medio, enorme, y está ' +
               'respirando. Lo abrazo. Me quedo pegada al tronco respirando ' +
               'cuando él respira, y no hago nada más. Eso alcanza.',
    ...
    puerta: {
      ...
      /* Era un experimento curioso —"a ver hasta donde llega esto"— y no
         decia nada de ella. Abrir una y otra vez sabiendo perfectamente que
         del otro lado no esta lo que buscas es otra cosa, y es suya. */
      esconde: 'La abro y del otro lado está el mismo campo. La cierro y la ' +
               'vuelvo a abrir, y ahora hay una habitación vacía. La cierro ' +
               'otra vez. Sé perfectamente que del otro lado no va a estar lo ' +
               'que busco, y la vuelvo a abrir igual. Y otra vez. Y otra.',
      revela: 'casa'
    },
    ruina: {
      nombre: 'Lo que quedó',
    ...
    bandada: {
      ...
      vuelta: 'Los pájaros otra vez, cruzando para el mismo lado. Puede que ' +
              ...
              'cortita.',
      /* Era un truco de magia bien contado y nada mas. El remate le da lo que
         le faltaba: en un lugar donde ella no maneja nada, esto le obedece. */
      esconde: 'Ninguno bate las alas al mismo tiempo que otro, salvo cuando ' +
               'los miro. Cuando los miro se ordenan; cuando aflojo la vista ' +
               'se desarman otra vez. Pruebo tres veces para estar segura y ' +
               'las tres me hacen caso. Es lo único que últimamente me hace caso.',
      revela: 'casa'
    },
    barca: {
      nombre: 'La barca',
  ...
  /* Las cuatro versiones arrancan con la MISMA frase, y por eso se cambian
     juntas: "abri los ojos a la madrugada". Antes decia "a las cuatro y
     monedas", que da la hora exacta del insomnio pero se lee como un dato; "a
     la madrugada" es como se cuenta esto en voz alta. */
  function final(indicios, recorrido) {
    ...
    if (n === 0) {
      return {
        titulo: 'Me desperté',
        partes: [
          'Abrí los ojos a la madrugada. Mi cuarto, mi techo, la ' +
    ...
    if (n <= 2) {
      return {
        ...
        partes: [
          'Abrí los ojos a la madrugada, con la cabeza en una cosa ' +
    ...
    if (n <= 4) {
      return {
        ...
        partes: [
          'Abrí los ojos a la madrugada y me quedé quieta, juntando las ' +
    ...
    return {
      ...
      partes: [
        ...
        'Abrí los ojos a la madrugada, despierta de una, como si no ' +
    ...
    if (n >= 5) {
      return {
        ...
        parrafos: [
          ...
          'mayoría no se queda.',
          /* Sin "no es magia y no hace falta que lo sea". Se lo estaba
             diciendo a una astrologa: en su boca esa frase deja de ser humilde
  ...
  /* La carta de papel. La escribio Nico, palabra por palabra: es la unica
     ...
     Lo unico que se toco, y a pedido suyo, fueron tres ecos: "de una forma u
     otra" estaba dos veces, "un poquito de paz" chocaba con "de a poquito" en
     el mismo parrafo, y la lista de deseos —sonreir, disfrutar, hacer planes—
     se repetia casi textual en el parrafo siguiente. Sacar un eco no es
     emprolijar: la frase repetida se lee la segunda vez como si no se hubiera
     dicho la primera, y eso le baja el peso a las dos. */
  var CARTA_PARA_BEL = {
    parrafos: [
      'Bel:',
      ...
      /* "que es" y no "que sigue siendo". Igual que cuando salio "de una forma
         u otra": el verbo en continuo da por supuesto que algo cambio y deja
         entrever una distancia que la frase no necesita nombrar. En presente
         no explica nada, y por eso no abre esa puerta. */
      'Simplemente quería hacer algo distinto para alguien que es muy importante para mí.',
      ...
      'Sé que atravesaste cosas muy difíciles, especialmente estos últimos meses. Y la verdad es que siento que no hay mucho que pueda decir sobre eso que realmente alcance.',

      'Solamente deseo que, con el tiempo, puedas encontrar un poco de paz. Que los recuerdos lindos puedan empezar a pesar más que el dolor. Y que, de a poquito, puedas volver a sonreír, disfrutar de las cosas, hacer planes, reírte mucho y sentirte orgullosa de la persona que sos.',

      'Porque creo que hay personas que dejan una parte de ellas en nosotros que permanece para siempre. Y quizás una de las formas más lindas de llevarlas con nosotros sea justamente esa: seguir viviendo así, y hacer de eso nuestro motor, también por ellas.',

      'Ojalá sigas creciendo, cumpliendo tus sueños y construyendo esa vida llena de cosas lindas que merecés.',

      'Por mi parte, me quedo con las cosas que compartimos y con todos esos momentos en los que simplemente la pasamos tan lindo. Me quedo con todo lo que vivimos y, sobre todo, con el cariño enorme que te tengo.',

      'Quiero que sepas que te quiero mucho, que sos una persona muy importante para mí y que, pase lo que pase, siempre vas a poder contar conmigo.',
      ...
      /* Sin "de una forma u otra", igual que en la frase de mas arriba: matiza
         el "siempre voy a querer" justo cuando lo esta diciendo. */
      'Y gracias por ser vos. Por ser esa persona única y especial que conocí y que siempre voy a querer.',
