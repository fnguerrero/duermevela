...
(function () {
  ...
  function jugar(c, elCarta) {
    ...
    /* Lo que hizo la carta se dice AHORA, mientras el lugar se esta
       transformando, y no cuando la transformacion termino.

       Las catorce frases hablan del lugar que se va —"se encendio", "le
       crecio una vela", "dio la vuelta entera"— y ninguna lo nombra, porque
       la misma frase sirve saliendo de cualquier lado. Dichas al final, el
       sujeto ya no estaba en pantalla: uno leia "se encendio" mirando una
       casa que acababa de aparecer, y no habia con que atar ese "se". Nico
       pregunto por tres distintas en una sola partida.

       Dichas aca, la montaña rusa todavia se esta volviendo casa y el sujeto
       esta a la vista. No se toco ni una palabra de las catorce. */
    decir(c.accion);
    ...
    /* La UNICA puerta por la que el paso avanza, con guarda propia.

       Antes habia dos caminos: este y el del guardian de mas abajo, cada uno
       con su lógica repetida y su propia guarda — o sea ninguna guarda, porque
       una no sabia de la otra. El guardian mira `!J.destino`, y `J.destino` se
       limpia cuando termina la mutacion, no cuando el paso se cierra: entre
       una cosa y la otra queda la ventana del texto de la carta y el respiro
       de 1100 ms. Si el guardian saltaba ahi —y salta, porque su contador va
       en tiempo real mientras esa ventana va por la cola de relojes, que en
    ...
    J.avanzarPaso = function () {
      ...
      /* Normalmente el bucle de dibujo ya lo movio, apenas termino la
         ...
         que se este dibujando. Asignar de nuevo es inocuo y garantiza que el
         paso avance igual. */
      J.lugar = destino;
      if (J.recorrido[J.recorrido.length - 1] !== destino) J.recorrido.push(destino);
    ...
    J.seguirPaso = function () {
      ...
      /* La frase de la carta ya se dijo al arrancar la mutacion, asi que aca
         ...
         mal: llegaba con el sujeto ya fuera de pantalla, y ademas empujaba el
         texto del indicio —que se acababa de leer— antes de tiempo. */
      pedirSeguir(function () {
        luego(1100, J.avanzarPaso);
      }, tiempoDeLectura(c.accion) * .45);
    };
  ...
  function vigilarPaso(dt) {
    ...
    abandonado = 0;
    /* Por la misma puerta que el cierre normal, no por un camino paralelo: la
       guarda de alla es la que evita que el paso avance dos veces, y hasta
       ahora este pasaba por al lado. Si todavia no se jugo ninguna carta no
       hay puerta, y ahi si corresponde el cierre a mano. */
    if (J.avanzarPaso) { J.avanzarPaso(); return; }
  ...
  function abrirCarta() {
    ...
    /* Primero entra la carta y DESPUES se saca el sobre, no al reves.

       Estaba escrito al reves: se sacaba el sobre y la carta llegaba 900 ms
       mas tarde. Como las dos capas se desvanecen —el sobre en 1,6 s y la
       carta en 1,8— en ese hueco ninguna de las dos estaba entera y las dos
       son casi opacas: por el medio se veia la escena del juego que quedo
       abajo. Nico lo vio y pregunto si volver a la cama antes de abrir la
       carta era a proposito. No lo era: era un agujero.

       La carta esta en la capa de arriba, asi que entrando primero tapa al
       sobre mientras el sobre sigue entero. Cuando el sobre se va, ya no lo
       ve nadie. */
    Audio2.volteo();
    elCarta.classList.add('ver');
    elCarta.scrollTop = 0;
    luego(1900, function () { elSobre.classList.remove('ver'); });
  }
